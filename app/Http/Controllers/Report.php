...
class Report extends Controller
{
    ...
    public function index(Request $request){
        ...
        //     SELECT p.nama_produk,o.id_produk,COUNT(*) AS total FROM `t_order` o JOIN t_produk p WHERE p.id_produk = o.id_produk GROUP BY id_produk) AS t GROUP BY t.id_produk ORDER BY total DESC
            $penjualanToko = DB::select("SELECT a.id_user ,s.nama_toko ,s.logo_toko, a.tgl_selesai ,COUNT(a.total) AS total FROM(SELECT k.id_user, mo.tgl_selesai, k.id_produk AS total FROM `t_keranjang` k , t_multi_order mo WHERE k.kode_keranjang = mo.kode_keranjang AND mo.order_status ='4' AND month(mo.tgl_selesai) = month(now()) AND year(mo.tgl_selesai) = year(now()) UNION ALL SELECT o.id_user, o.tgl_selesai, o.id_user AS total FROM t_order o WHERE month(o.tgl_selesai) = month(now()) AND year(o.tgl_selesai) = year(now())) AS a , t_setting s WHERE a.id_user = s.id_user GROUP BY a.id_user ORDER BY total DESC LIMIT 5");
